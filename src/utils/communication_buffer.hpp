...
CommBuffer<T>::~CommBuffer() {
  ...
  if (my_request_.use_count() == 1) { // This is the last shallow copy of this buffer
    ...
    auto ms_start = Logging::GetMsSince(0);
    ...
    auto ms_dura = Logging::GetMsSince(ms_start);
    if (ms_dura > 10) {
      Logging::LogThingsInCommBuffer(state_, comm_type_, buf_.size(), started_irecv_, my_rank, tag_,
                                     send_rank_, recv_rank_, active_);
...
void CommBuffer<T>::FinishSend() {
  ...
  if (ms_dura > 10) {
    Logging::LogThingsInCommBuffer(state_, comm_type_, buf_.size(), started_irecv_, my_rank, tag_,
                                   send_rank_, recv_rank_, active_);
    std::cout << "CommBuffer FinishSend took " << ms_dura << "ms" << std::endl;
  }
}
