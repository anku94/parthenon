#include "communication_buffer.hpp"
...
template <class T>
void CommBuffer<T>::DrainPendingSends(SendDrainQueue &dq) {
  // Give up ownership only of MPI_Request structs if sends are pending.
  // We don't give up ownership of the buffers, as we assume that this will
  // only be called after the receiver has received the actual data.

  if (*comm_type_ != BuffCommType::sender) {
    return;
  }

  if (*state_ != BufferState::sending && *state_ != BufferState::sending_null) {
    return;
  }
  ...
  if (!flag) {
    dq.AddSendRequest(std::move(my_request_));
    my_request_ = std::make_shared<MPI_Request>(MPI_REQUEST_NULL);
  }
...
template <class T>
bool CommBuffer<T>::IsSendPending() {
  if (*comm_type_ != BuffCommType::sender) {
    return false;
  }
...
template <class T>
std::shared_ptr<MPI_Request> CommBuffer<T>::ReleaseMPIReq() {
  if (*comm_type_ != BuffCommType::sender) {
    std::stringstream msg;
    msg << "Trying to release MPI request from a non-sender buffer. "
        << "CommType: " << CommBufferLogging::BufCommType2Str(*comm_type_);
    std::cout << msg.str() << std::endl;
  }

  if (*state_ != BufferState::sending && *state_ != BufferState::sending_null) {
    std::stringstream msg;
    msg << "Trying to release MPI request from a buffer that is not in the sending "
           "state. "
        << "State: " << CommBufferLogging::BufState2Str(*state_);
    std::cout << msg.str() << std::endl;
  }

  auto req = my_request_;
  my_request_ = std::make_shared<MPI_Request>(MPI_REQUEST_NULL);
  *state_ = BufferState::stale;
  ...
  if (*comm_type_ == BuffCommType::receiver ||
    ...
    if (*started_irecv_) {
      ...
      if (flag != 0 and flag != 1) {
        std::cout << "Weird value of flag: " << flag << std::endl;
      }
      if (flag) {
...
template <class T>
void CommBuffer<T>::SendGap() {
  static uint64_t sleep_ns = 0;
  if (sleep_ns == 0) {
    const char *env = std::getenv("PARTHENON_SEND_GAP_NS");
    if (env) {
      sleep_ns = std::stoul(env);
    } else {
      sleep_ns = 1;
    }
  }
...
template class CommBuffer<parthenon::ObjectPool<Kokkos::View<double*, Kokkos::LayoutRight, Kokkos::HostSpace>>::owner_t>;
} // namespace parthenon
